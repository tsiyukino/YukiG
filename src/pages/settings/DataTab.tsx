...
export default function DataTab() {
  ...
  return (
    <SettingsSection icon={<FolderOpen size={15} />} title="Storage">
      ...
      >
        <div className={s.dataDirBlock}>
          <div className={s.dataDir}>
            <span className={s.dataDirPath}>{dataDir ?? "…"}</span>
            <button
              className={s.btn}
            ...
            >
              {migrating ? <RefreshCw size={13} className={s.spin} /> : <FolderOpen size={13} />}
          ...
          {migrateError && <span className={s.error}>{migrateError}</span>}
          {openDirError  && <span className={s.error}>{openDirError}</span>}
        </div>
      </SettingsRow>
      <SettingsRow
        label="Database file"
      ...
      >
        <span className={s.mono}>{dataDir ? `${dataDir}\\filevault.db` : "…"}</span>
      </SettingsRow>
      <SettingsRow
        label="Thumbnail cache"
        description="Cached image thumbnails generated from item folders."
      >
        <span className={s.mono}>{dataDir ? `${dataDir}\\thumbnails\\` : "…"}</span>
      </SettingsRow>
    </SettingsSection>
  );
}
